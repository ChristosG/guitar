...
# ---------------------------------------------------------------------------
# "who" — level + language; the roster left the product
# ---------------------------------------------------------------------------

def test_level_and_language_are_the_whole_answer(db):
    """The answer keeps the historical `answers["who"]` shape (student keys
    pinned to None) because `generate_interview_outline` and `_answer_confirm`
    read those keys literally."""
    interview = _start(db)

    r = answer_interview(db, interview, {"level": "all_levels", "language": "el"})
...
def test_the_who_step_offers_no_roster_even_when_student_rows_exist(db):
    """The seed DB of a fresh desktop install still carries historical student
    rows. The wizard must never surface them — the feature left the product."""
    ...
    assert "Elena" not in str(state)
    assert "all_levels" in state["findings"]["levels"]
    assert state["findings"]["languages"] == ["el", "en"]


def test_the_language_is_asked_explicitly_and_normalized(db):
    """An English course from a Greek cockpit is the NORMAL case (5 of 6 real
    courses) — the wizard asks, it does not inherit the UI locale. Junk is
    normalized/defaulted, never a 500 (the VALIDATION contract)."""
    for raw, expected in [("en", "en"), ("EN", "en"), ("el-GR", "el"),
                          ("xx", "el"), (42, "el"), (None, "el")]:
        interview = _start(db)
        r = answer_interview(db, interview, {"level": "beginner", "language": raw})
        assert r["ok"], raw
        assert interview.answers["who"]["language"] == expected, raw


def test_a_legacy_student_id_answer_is_ignored_not_reasked(db):
    """A crash-resumed interview from before the roster removal may re-submit
    the old shape. Refusing it would strand the wizard on a question the UI
    can no longer render — so `student_id` is ignored, never validated."""
    interview = _start(db)
    r = answer_interview(
        db, interview, {"student_id": str(uuid.uuid4()), "level": "beginner", "language": "en"},
    )
    assert r["ok"]
    assert interview.step == "duration"
    assert interview.answers["who"]["student_id"] is None
    assert interview.answers["who"]["level"] == "beginner"
    assert interview.answers["who"]["language"] == "en"


def test_an_unknown_level_reasks(db):
    interview = _start(db)
    r = answer_interview(db, interview, {"level": "wizard"})
...
def test_level_and_language_reach_the_materialized_course(db):
    """The two facts the who step still collects must land where generation and
    the UI read them: `target_profile.level` (curricula-list badge, DOCX
    subtitle) and `Block.language` (every draft prompt's language directive).
    This is the regression guard for the roster removal — if the step's answer
    key contract drifts, both silently fall back (level blank, language "el")
    and no error ever fires."""
    source = _source(db)

    interview = _start(db)
    answer_interview(db, interview, {"level": "beginner", "language": "en"})
    answer_interview(db, interview, {"weeks": 8, "sessions_per_week": 1,
                                     "minutes_per_session": 50})
    ...
    course = db.get(Block, interview.root_id)
    assert course.meta["student_id"] is None
    assert course.target_profile["level"] == "beginner"
...
def test_post_curricula_interview_starts_at_who(client, db):
    # A student row in the DB (the shipped seed carries historical ones) must
    # not leak into the wire response — the roster left the product.
    ...
    assert body["findings"]["languages"] == ["el", "en"]
